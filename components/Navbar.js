    ...
    <nav className="bg-white shadow-sm sticky top-0 z-50">
      <div className="max-w-7xl mx-auto px-4 lg:px-6 py-3 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2 lg:gap-3 min-w-0">
          <img src="/images/logo-rsud.png" alt="Logo RSUD Caruban" className="h-9 lg:h-12 shrink-0" />
          <img src="/images/logo-madiun.png" alt="Logo Kabupaten Madiun" className="h-9 lg:h-12 shrink-0" />
          <div className="min-w-0">
            <p className="font-bold text-primary text-sm lg:text-lg leading-tight truncate">
              Gizi_RSCaruban
            </p>
            <p className="text-[10px] lg:text-xs text-gray-500 truncate">Caring for Your Health</p>
          </div>
        </div>

        {/* Menu - hanya tampil di layar lebar (laptop/desktop) */}
        <div className="hidden lg:flex items-center gap-8 text-sm font-medium text-gray-700">
          {menuItems.map((item) => (
            <a key={item.label} href={item.href} className="hover:text-primary whitespace-nowrap">
              {item.label}
            </a>
          ))}
        </div>
        ...
        <div className="flex items-center gap-2 shrink-0">
          {/* WhatsApp - teks lengkap hanya di layar lebar, ikon saja di HP & tablet */}
          <a
            href="https://wa.me/6281231337897"
            target="_blank"
            rel="noopener noreferrer"
            className="bg-primary text-white p-2.5 lg:px-4 lg:py-2 rounded-full lg:rounded-lg text-sm font-medium hover:bg-primary-dark flex items-center gap-2"
          >
            ...
            <span className="hidden lg:inline whitespace-nowrap">WhatsApp: 0812-3133-7897</span>
          </a>

          {/* Tombol hamburger - muncul di HP & tablet (di bawah lg) */}
          <button
      ...
      {/* Menu dropdown - HP & tablet saja, muncul kalau menuOpen true */}
      {menuOpen && (
